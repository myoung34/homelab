...
resource "talos_machine_configuration_apply" "controlplane" {
  ...
  config_patches = [
    templatefile("${path.module}/templates/install-disk-and-hostname.yaml.tmpl", {
      ...
      kubernetes_version = try(each.value.kubernetes_version, "") == "" ? local.kubernetes_version : each.value.kubernetes_version
    }),

    templatefile("${path.module}/templates/controlplane.yaml.tmpl", {
      talos_version         = try(each.value.talos_version, "") == "" ? local.talos_version : each.value.talos_version
      kubernetes_version    = try(each.value.kubernetes_version, "") == "" ? local.kubernetes_version : each.value.kubernetes_version
